...
def handle_message(data):
    ...
    for row in income_data:
        ...
        engine_capacity_rounded = round(float(engine_capacity) / 100.0) * 100
        
        if row['Brand:'] == BRAND and row['Model:'] == MODEL and row['Year of Manufacture:'] == YOM and row['Transmission:'] == TRANSMISSION and row['Body type:'] == BODY and row['Fuel type:'] == FUEL and engine_capacity_rounded == round(float(ENGINE) / 100.0) * 100:
    ...
    if len(data) == 1:
        ...
        cmd_collection = db["predictions"]  
        cmd_collection.replace_one({}, {'price': CMD }, upsert=True)
